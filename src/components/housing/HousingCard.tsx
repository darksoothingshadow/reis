      ...
      <div className="card-body gap-2">
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="badge badge-outline badge-sm">
              {t(`housing.roomType.${post.roomType}`)}
            </div>
            <h3 className="mt-1 text-base font-semibold">{post.district}</h3>
          </div>
          {post.priceCzk !== null && (
            <div className="text-right text-sm font-semibold">
              {post.priceCzk.toLocaleString(locale)}{' '}
              <span className="text-xs font-normal opacity-70">{t('housing.priceUnit')}</span>
            </div>
          )}
        </div>
        ...
        <div className="card-actions items-center justify-between">
          ...
          <button
            ...
            onClick={async () => {
              setReportState((await onReport(post)) ? 'sent' : 'failed');
            }}
          >
            <Flag size={12} />{' '}
            {reportState === 'sent'
